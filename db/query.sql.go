...
const deleteVisitByID = `-- name: DeleteVisitByID :exec
DELETE FROM Visit WHERE id = ?
`

func (q *Queries) DeleteVisitByID(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, deleteVisitByID, id)
	return err
}
...
const getVisitOwnerByID = `-- name: GetVisitOwnerByID :one
SELECT UserId FROM Visit WHERE id = ?
`

func (q *Queries) GetVisitOwnerByID(ctx context.Context, id int64) (sql.NullInt64, error) {
	row := q.db.QueryRowContext(ctx, getVisitOwnerByID, id)
